### The seasonplot function is taken from the forecast package (version 3.22)
...
make.seasonplot = function(vars) {
    ...
    title = paste("Seasonal plot for", vars$currentName)

	opar = par(mfrow = c(1, 2), cex.axis = 0.9)
	on.exit(par(opar))

    seasonplot(vars$tsObj, freq, col = cols, pch = 19,
               year.labels = TRUE, year.labels.left = TRUE,
               main = title)

	vars = decomposition(vars)
	season = vars$decompVars$components[,"seasonal"]
