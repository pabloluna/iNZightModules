...
### decomposition returns a modified list 'vars' with a component called
### decompVars, which is a list containing variables we need for some of
### the other functions.

decomposition = function(vars, trendCol = "black", seasonCol = "#45a8ff",
                         randCol = seasonCol) {
    vars = make.ts(vars)
    
    xlist = get.x(vars$tsObj)
    x = xlist$x
    x.units = xlist$x.units
    
    
    if (vars$freq > 1) {
        decomp = stl(vars$tsObj, "periodic")
    }
    else {
        trend.comp = loess(vars$data[,1] ~ x)$fitted + vars$tsObj * 0
        residuals.comp = vars$tsObj - trend.comp
        seasons.comp = vars$tsObj * 0
        decomp = list()
    ...
    max.index = ifelse(vars$freq > 1, which.max(y.season), 1)
    season.vp.y[max.index] = max(y.season) + expandBy
    if (vars$freq == 1) season.vp.y[2] = -expandBy
    ...
    grobs$statusText = textGrob(paste("Decomposition of data:", vars$currentName),
    ...
    decompVars = list(tree = image, ranges = ranges, props = props,
                      raw = vars$tsObj@.Data, components = decomp$time.series,
                      vertMargins = vertMargins)
    vars$decompVars = decompVars
    vars
}
...
plot.decomposition = function(vars) {
