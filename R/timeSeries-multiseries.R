...
### Time series trend and seasonal stacking.

### There are 2 functions: one for time series data with a
### frequency greater than 1 and one for the cases when
### frequency = 1.

multiseries = function(vars) {
    if (vars$freq > 1) { 
        multiseries.2p(vars)
    } else {
        multiseries.1(vars)
...
multiseries.2p = function(vars) {
    varNums = 1:length(names(vars$data))
    ...
    varNames = character(length(varNums))
    for (i in 1:length(varNums)) {
        vardata = vars$data[, i, drop = FALSE]
        curr.vars = vars
        curr.vars$data = vardata
        curr.vars = decomposition(curr.vars)
        name = gsub("[[:space:]]+", "_", curr.vars$currentName)
        varNames[i] = curr.vars$currentName
