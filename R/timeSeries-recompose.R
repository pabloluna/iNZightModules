...
### The function shiftLineUp moves a linesGrob (copy) upwards within the plots.vp
### viewport. It is moved up so that the first point of the lineGrob coincides with
### the first point of the component we are going to add it to.

shiftLineUp = function(vars.decomp, vpName, destination.y.parent, n.steps = 15, animate = TRUE, e = NULL) {
...
### The function add.seasonal adds the seasonal component to the trend component.
### It adds to a copy of the trend line so that the original trend line is
### preserved in the plot. The copy of the season lineGrob is removed as the joint
### line is drawn. After the joint line is done, a copy of it is added to the plot,
### for use in add.random.

add.seasonal = function(trend.attr, vars.decomp, animate = TRUE, e = NULL) {
...
### The function add.random adds the random component to the trend+seasonal component.
### It adds to a copy of the trend+seasonal line so that the trend+seasonal line is
### preserved in the plot. The copy of the random lineGrob is removed as the joint
### line is drawn.

add.random = function(trend.attr, vars.decomp, animate = TRUE, e = NULL) {
    decomp = vars.decomp$decompVars
...
### The recompose function takes the decomposition and then "recomposes" it.
### First we add copies of the lineGrobs to the plots.vp viewport. Then we shift
### the copy of the seasonal component upwards and add it to the trend component
### and we get a trend + seasonal line at the top of our plot. Then we shift the
### copy of the random component upwards and add it to the trend+seasonal component
### and we get the raw time series at the top of our plot. Recompose takes one
### argument decomp, which is a list returned by decomposition(tsObj).

recompose = function(vars.decomp, animate = TRUE, e = NULL) {
    ...
    tree = addGrob(tree, trendLabel)
    vars.decomp$decompVars$tree = tree
    vars.decomp = shiftLineUp(vars.decomp, "random", trendSeason.y.parent,
                              n.steps = if (animate) 1 else 30, animate = animate, e = e)
