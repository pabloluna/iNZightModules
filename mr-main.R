...
multipleResponseWindow <- function(e) {
  ...
  topLayout[2, 1:2,anchor = c(-1,-1),expand=TRUE] <- glabel("Type the name for a new MR object")
  topLayout[4, 1] <- (mrName <- gedit(initial.text = "mro1"))
  topLayout[4, 2] <- gbutton("Create MR object",
                             handler = function(h, ...) {
                               ...
                               if(test.result) gmessage(paste("the",names(test.frm)[test.index[1]],"make the process down!"))
  ...
  topLayout[5, 1:2,anchor = c(-1,-1),expand=TRUE] <- glabel("Choose multiple response object")
  ...
  topLayout[9, 1, anchor = c(-1,-1),expand=TRUE] <- glabel("Broken out by:")
    ...
    (breakOutSelect <- gcombobox(c("   ",names(tag(mrWin, "dataSet"))),selected=0,
                                 editable = FALSE,
                                 handler = function(h, ...) {
  ...
  topLayout[11, 2:3, expand = TRUE] <-
    (breakOutSelect2 <- gcombobox(c("   ",names(tag(mrWin, "dataSet"))),selected=0,
                                  editable = FALSE,
                                  handler = function(h, ...) {
  ...
  topLayout[14, 3, expand = TRUE] <-
    gbutton("Combinations", handler = function(h, ...) {
      ...
      currentObj <- tag(mrWin, "mrObjects")[[svalue(mrSelect)]]
      browser()
      mrSummaryWindow(capture.output(
        plotcombn(by(currentObj,
                     as.formula(paste("~", svalue(breakOutSelect))),
                     mroPara))
      ))
    })
